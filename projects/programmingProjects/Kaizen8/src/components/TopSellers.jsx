...
function TopSellers({ products, onAddToCart }) {
  ...
  try {
    ...
    await axios.post(`http://localhost:4000/order/${product.id}`, {
      quantity: 1,
    })

    product.salesCount += 1 // 🟢 update count locally so UI re-renders

    await refreshProducts() // optional: keeps everything in sync later
  } catch (err) {
